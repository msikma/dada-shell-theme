...
set dst_library "/Volumes/Files/Backups/$dada_hostname/Library"

check_rsync_version $name
check_hostname $name
check_needed_dirs $name 'source' $src_files $src_desktop $src_documents $src_library
check_needed_dirs $name 'target' $dst_files $dst_desktop $dst_documents $dst_library

print_backup_start $purpose $name $dada_hostname
print_last_backup_time $name
print_backup_dirs $src_files $dst_files $src_desktop $dst_desktop $src_documents $dst_documents $src_library $dst_library
